...
func TestSetupGenesis(t *testing.T) {
	var (
		...
		customg     = Genesis{
			Config: &params.ChainConfig{SangamBlock: big.NewInt(3)},
			Alloc: GenesisAlloc{
	...
	oldcustomg.Config = &params.ChainConfig{SangamBlock: big.NewInt(2)}
	tests := []struct {
		name       string
		...
		},
		{
			...
			fn: func(db ethdb.Database) (*params.ChainConfig, common.Hash, error) {
				// Commit the 'old' genesis block with Sangam transition at #2.
				// Advance to block #4, past the sangam transition block of customg.
