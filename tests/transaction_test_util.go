...
func (tt *TransactionTest) Run(config *params.ChainConfig) error {
	validateTx := func(rlpData hexutil.Bytes, signer types.Signer, isSangam bool, isIstanbul bool) (*common.Address, *common.Hash, error) {
		...
		// Intrinsic gas
		requiredGas, err := core.IntrinsicGas(tx.Data(), tx.To() == nil, isSangam, isIstanbul)
		if err != nil {
			return nil, nil, err
		}
	...
	for _, testcase := range []struct {
		...
		{"Sangam", types.SangamSigner{}, tt.Sangam, true, false},
		{"EIP150", types.SangamSigner{}, tt.EIP150, true, false},
		...
		sender, txhash, err := validateTx(tt.RLP, testcase.signer, testcase.isSangam, testcase.isIstanbul)
