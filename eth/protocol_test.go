...
func TestForkIDSplit(t *testing.T) {
	var (
		...
		configNoFork  = &params.ChainConfig{SangamBlock: big.NewInt(1)}
	...
	// Progress into Sangam. Fork's match, so we don't care what the future holds
	...
	select {
	case err := <-errc:
		t.Fatalf("sangam nofork <-> profork failed: %v", err)
	case <-time.After(250 * time.Millisecond):
		p2pNoFork.Close()
		p2pProFork.Close()
	}
