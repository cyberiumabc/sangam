...
public class AndroidTest extends InstrumentationTestCase {
	...
	// Tests that recovering transaction signers works for both Sangam and EIP155
	...
	public void testIssue14599() {
		try {
			...
			preEIP155.getFrom(null);           // Sangam should accept sangam
			preEIP155.getFrom(new BigInt(4));  // EIP155 should accept sangam (missing chain ID)
			postEIP155.getFrom(new BigInt(4)); // EIP155 should accept EIP 155

			try {
				postEIP155.getFrom(null);
				fail("EIP155 transaction accepted by Sangam");
			} catch (Exception e) {}
		} catch (Exception e) {
			fail(e.toString());
		}
